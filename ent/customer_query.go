...
func (cq *CustomerQuery) QueryDeposits() *DepositQuery {
	query := (&DepositClient{config: cq.config}).Query()
...
func (cq *CustomerQuery) Clone() *CustomerQuery {
	...
	return &CustomerQuery{
		...
		withDeposits: cq.withDeposits.Clone(),
...
func (cq *CustomerQuery) WithDeposits(opts ...func(*DepositQuery)) *CustomerQuery {
	query := (&DepositClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withDeposits = query
	return cq
}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Customer, error) {
	...
	if query := cq.withDeposits; query != nil {
		if err := cq.loadDeposits(ctx, query, nodes,
			func(n *Customer) { n.Edges.Deposits = []*Deposit{} },
			func(n *Customer, e *Deposit) { n.Edges.Deposits = append(n.Edges.Deposits, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CustomerQuery) loadDeposits(ctx context.Context, query *DepositQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *Deposit)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Customer)
	nids := make(map[uuid.UUID]map[*Customer]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(customer.DepositsTable)
		s.Join(joinT).On(s.C(deposit.FieldID), joinT.C(customer.DepositsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(customer.DepositsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(customer.DepositsPrimaryKey[0]))
